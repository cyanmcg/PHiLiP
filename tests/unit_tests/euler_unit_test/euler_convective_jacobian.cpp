        ...
        for (unsigned int v=0; v < dealii::GeometryInfo<dim>::vertices_per_cell; ++v) {
            ...
            for (int d=0; d<dim; d++) {
                ...
                for (int col=0; col<nstate; col++) {
                    ...
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_plus2 = euler_physics.convective_flux(soln_plus2,soln_gradient);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_plus = euler_physics.convective_flux(soln_plus,soln_gradient);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_mins = euler_physics.convective_flux(soln_mins,soln_gradient);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_mins2 = euler_physics.convective_flux(soln_mins2,soln_gradient);
